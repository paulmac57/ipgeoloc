...
if __name__ == "__main__":
    ...
    for measurement in measurements:
        # temporary fix, this probe has poor results so leave it out
        if measurement[1] != '25200': 
    ...
    for measurement in measurements:
        ...
        # Create the Dictionary that will hold the data on the closest facility to the target
        facility_dict = {}
        ...
        for source_probe in probe_list:
            ...
            if hops != []:
                ...
                max_col = 50 # limits the number of hops, place the hop results in the spreadsheet starting at column 51
                ...
                if last_router_hop and ixp_hop:
                    ...
                    if measurement_dict[this_measurement][source_probe]['ixp_to_last_router_time'] < min_i_to_lr_time:
                        ...
                        min_i_to_lr_row = row
                        measurement_dict[this_measurement][source_probe]['ixp_ip'] = measurement_dict[this_measurement][
                        source_probe][ixp_hop]['ip']
                ...
                # if the ixp_hop and the Last hop router are the same then time will be zero
                # but this could still be a great row to choose so set time to be i to target instead
                if ixp_hop == last_router_hop:
                    min_i_to_lr_time = min_i_to_t_time
        ...
        results_dict[target_ip_address]['target_to_lr'] = tl_time
        print('target_ip',target_ip_address)
        print('target_id', target_id)
        print ('target lat,lon',lat_tgt,lon_tgt )
        ...
        print('source ip',measurement_dict[this_measurement][probe]['ip'])
        print('source lat',measurement_dict[this_measurement][probe]['lat'])
        print('source lon',measurement_dict[this_measurement][probe]['lon'])
        last_router_hop=measurement_dict[this_measurement][probe]['last_router_hop']
        ixp_hop=measurement_dict[this_measurement][probe]['ixp_hop']
        overall_ws.write(
        ...
        if ixp_hop:
            ipaddress = measurement_dict[this_measurement][probe][ixp_hop]['ip']
            #ipaddress = '196.223.14.47' # REMOVE
            subnet= ipaddress.split('.')
            subnet=subnet[0]+'.' + subnet[1]+'.'+subnet[2]
            ...
            print('source_probe',probe)
            # Temporary fix for 196.10.140.0 range as this is in mid transfer to 196.60.70.0
            # see https://www.pch.net/ixp/details/1308
            if subnet ==  '196.10.140':
                f = []
                fac_id = []
                f.append('1225')
                print(f)
                fac_id.append(f) 
                print(fac_id)
            # Temporary fix for 105.22.46.69 range as this is not showiing in peeringdb
            # see https://www.pch.net/ixp/details/1308
            elif subnet ==  '105.22.46':
                f = []
                fac_id = []
                f.append('850')
                print(f)
            ...
            # Temporary fix for 196.223.22.101 range as this is not showiing in peeringdb
            # see https://www.pch.net/ixp/details/1308
            elif ipaddress ==  '196.223.22.101':
                f = []
                fac_id = []
                f.append('760')
                print(f)
                fac_id.append(f) 
                print(fac_id)
            # Temporary fix for 196.223.30.101 range as this is not showiing in peeringdb
            # see https://www.pch.net/ixp/details/1308
            elif ipaddress ==  '196.223.30.101':
                f = []
                fac_id = []
                f.append('3625')
                print(f)
                ...
                net_id = peeringdb_cur.fetchone()[0]
                print('net_id',net_id)
                sql = 'select fac_id from peeringdb_network_facility where net_id = ?'
                val = (str(net_id),)
                peeringdb_cur.execute(sql,val)
                fac_id = peeringdb_cur.fetchall()    


            print('Fac_id',fac_id,type(fac_id))
            ...
            # if more than one facility exists then things get a bit harder to geolocate
            if len(fac_id) > 1:
                print("**********************************************************************")
                print('WARNING Multiple FACILITIES')
                print("FAC_ID", fac_id,type(fac_id))
                print("**********************************************************************")
                i = 0
                za_facs = []
                for fac in fac_id:
                    ...
                    print (info)
                    if info[0][10] == 'ZA':
                        za_facs.append(info[0])
                print(za_facs)  
                # if multiple South African Facilities exist then things get harder still  
                # We need to select the correct facility that the packets are exiting from.
                # To do this we use the fastest time from IXP to LHR, this should indicate where the packet
                # did not have to travel the large distances between facilities but in fact entered and exited from the same facility.
                # The IX_LAN_id will then give us the name and city of the IX. Buy Comparing this with each of the facilities
                # we should be able to work out which facility this was and retrieve the lat and lon coordinates of the correct 
                # facility.
                if len(za_facs) > 1:
                    print("**********************************************************************")
                    print('WARNING Multiple ZA FACILITIES')
                    print("ZA_FACS", za_facs)
                    print(target_ip_address)
                    print(target_id)
                    #print(source_info)
                    print(probe)
                    print(ixp_hop)
                    print(last_router_hop)
                    print(il_time)

            
                    print ('IXp Ip is', ipaddress )
                    print("**********************************************************************")
                    #print(measurement_dict[this_measurement][probe])
                    sql = 'select ixlan_id from peeringdb_network_ixlan where ipaddr4 = ?'
                    val = (str(ipaddress),)
                    peeringdb_cur.execute(sql,val)
                    ixlan_id = peeringdb_cur.fetchone()
                    print ('IX_LAN_ID', ixlan_id[0])
                    sql = 'select ix_id from peeringdb_ixlan where id = ?'
                    val = (str(ixlan_id[0]),)
                    peeringdb_cur.execute(sql,val)
                    ix_id = peeringdb_cur.fetchone()
                    print ('IX_ID', ix_id[0])
                    sql = 'select city from peeringdb_ix where id = ?'
                    val = (str(ix_id[0]),)
                    peeringdb_cur.execute(sql,val)
                    c = peeringdb_cur.fetchone()
                    city =c[0]
                    print ('city', city)
                    ...
                    for fac in za_facs:
                        ...
                        print("facility",fac)
                        f = fac[0]
                        f_city = fac[6] +' '+fac[7]+' '+fac[8] 
                        
                        if city in f_city:
                            print(f, city,f_city) 
                            city_facs.append(f)
                        print("city facs",city_facs)
                    if len(city_facs) != 1:
                        city_facs = city_facs[0]
                    else:
                        fac_id = []
                        fac_id.append(city_facs)
                        print(fac_id) 
                        #input("GREAT")
                else:
                    print(source_info)
                    fac_id = za_facs
                # if no Facilities exist in South Africa then skip (unlikely)
                if len(za_facs) == 0:
                    continue;
            # if no Facilities exist then skip (unlikely)
            if len(fac_id) == 0:
                    continue;
            # If only one facility exists then its easy
            print ('fac_id',fac_id)
            fac_id = fac_id[0][0]
            print ('FAC_id', fac_id)
            sql = 'select * from peeringdb_facility where id = ?'
            ...
            # Temporary fix for 105.22.46.69 range as this is in mid transfer to ????
            # see https://www.pch.net/ixp/details/1308
            elif subnet ==  '105.22.46':
                net_fac_id = '850'
            ...
            # see https://www.pch.net/ixp/details/1308
                ...
                val = (str(net_id),)
                peeringdb_cur.execute(sql,val)
                net_fac_id = peeringdb_cur.fetchone()[0]
                

            sql = 'select latitude,longitude from peeringdb_facility where id = ?'
            val = (str(net_fac_id),)
            print(net_fac_id)
            peeringdb_cur.execute(sql,val)

            myresult = peeringdb_cur.fetchone()
            print(myresult)
            print ("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
        ...
        # If probe is  not an anchor then have set radius to 1km as per RIPE info 
        if target_isanchor:
